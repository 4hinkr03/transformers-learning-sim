...
public class AutoBot extends Agent {
	...
	public void act(Planet planet, int step) {
		...
		if (locationIndex < path.size() - 1) {
			...
			// this means the autobot has reached the end of the path, meaning the optimised
			// path is safe
	...
	private Location nextLocation(Planet planet) {
		...
		if (!locations.isEmpty()) {
			...
			// corner case when all neighbours have been flagged, so no move can de done. We
			// can assume that
				...
				for (Location neighbour : neighbours) {
					if (nextLocation.matches(neighbour)) {
						...
						if (differenceTemp > difference) {
							...
							for (int step = nextIndexTemp - stepOffset; step < path.size()
									- stepOffset; step++, nextStep++) {
	...
	public boolean smoothPath(Planet planet) {
		// grids from min to max to smooth out path
		int gridMin = 2;
		int gridMax = 10;
		int failed = 0;
			...
			for (int i = 0; i < getPathSize() - grid; i++) {
				...
				if (vectorDist < dist) {
					if (Math.abs(v.x) > 1 || Math.abs(v.y) > 1) {
						// remove locations to be improved
						List<Location> tempPath = new ArrayList<>(path);
						for (int index = i + grid - 1; index > i; index--) {
							tempPath.remove(index);
						}
						// this means the start & finish locations are not next to each other, so the
						// gap removed must be filled with the smoothed solution
						// add better locations - backwards to ensure they are in teh right order in the
						// array
						// inverse vector so we can work from finish to start
						v.inverse();
						Location currentLoc = tempPath.get(i + 1);
						// remove start location because it will be added again during the vector
						// smoothing
						tempPath.remove(i);
					...
					// we only want to do this once and see if there is an improvement and sure it
					// doesn't get caught
	 ...
	 * @param start
	 * @param finish
	 * @return the distance between two locations using pythagoras
	 */
	private double getDistanceBetween(int start, int finish) {
		...
		for (int index = start; index < finish; index++) {
			...
			// System.out.println("start=" + startLoc + ", finish=" + finishLoc + ",
	 ...
	 * @param start
	 * @param finish
	 * @return the vector distance between two locations - shortest distance
	 */
	...
	public void die(Planet planet) {
		if (alive) {
			alive = false;
			// died so must flag current location
			flagLocation(planet, location);
			if (!previousPath.isEmpty()) {
				if (optimised || smoothed) {
					// revert back to old path, if it exists since the optimisation failed as we
					// died
					path.clear();
